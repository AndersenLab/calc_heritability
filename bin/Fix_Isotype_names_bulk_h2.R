...
# load trait file - this is different than in cegwas/nemascan because the input file is different
...
if(args[2] == "fix"){
    ...
    for(t in unique(colnames(traits)[2:length(colnames(traits))])) {
        ...
        fixed_names <- process_phenotypes(subset)
...
#     t_df <- fixed_names[,c(1,i+2)] ## this is different for h2
#     colnames(t_df)[2] <- gsub(colnames(t_df)[2], pattern = "\\.", replacement = "_")
#     trait_name <- colnames(fi)[2]
#     write.table(t_df, 
#                 file = glue::glue("pr_{trait_name}.tsv"),
#                 quote = F, col.names = T, row.names = F, sep="\t")
# }

write.table(unique(fixed_names$strain), 
